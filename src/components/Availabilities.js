...
export default class Availabilities extends React.Component {
  ...
  componentDidMount() {
    !this.state.loaded ? this.caller() : console.log("loaded");
  }
  ...
  handleNext = () => {
    this.props.history.push(
      `/teachers/${this.props.match.params.id}/availabilities/${this.addDays()}`
    );
  };
  handlePrevious = () => {
    this.props.history.push(
      `/teachers/${this.props.match.params.id}/availabilities/${this.subDays()}`
    );
  };
  ...
  render() {
    return (
      <div className="availabilitiesWrapper">
        ...
        {!("18022019" === this.props.match.params.date) ? (
          <button className="previousButton" onClick={this.handlePrevious}>
            Previous Week
          </button>
        ) : (
          ""
